...
require "../../tgl_indo.php";
	...
	if($bulan AND $tahun AND empty($status) ){
		$result = mysqli_query($kon, "SELECT * FROM `beli` INNER JOIN ongkir ON beli.idongkir = ongkir.idongkir INNER JOIN user ON beli.id = user.id WHERE MONTH(tglbeli) = '$bulan' AND YEAR(tglbeli) = '$tahun' ORDER BY tglbeli ASC");
	}else if($bulan AND $tahun AND $status){
		$result = mysqli_query($kon, "SELECT * FROM `beli` INNER JOIN ongkir ON beli.idongkir = ongkir.idongkir INNER JOIN user ON beli.id = user.id WHERE MONTH(tglbeli) = '$bulan' AND YEAR(tglbeli) = '$tahun' AND status = '$status' ORDER BY tglbeli ASC");
	}else{
